...
const LEVEL_ACCENT = 0x4dc6ff; // cyan/water untuk level 3
...
export class Level3Scene extends Phaser.Scene {
  ...
  private respawnX = LEVEL3_CONFIG.spawnX;
  private respawnY = LEVEL3_CONFIG.spawnY;
  ...
  create() {
    ...
    this.respawnX = LEVEL3_CONFIG.spawnX;
    this.respawnY = LEVEL3_CONFIG.spawnY;
    this.invulnerableUntil = 0;

    this.cameras.main.fadeIn(450, 0, 0, 0);

    const W = LEVEL3_CONFIG.width;
    const H = LEVEL3_CONFIG.worldHeight;

    // ---- Parallax bg ----
    this.parallax = new ParallaxBg(this);
    this.parallax.addLayer({ textureKey: TEX.BG_LEVEL3, scrollFactor: 0.3, alpha: 1 });
    ...
    // ---- Player ----
    this.player = new Player(this, LEVEL3_CONFIG.spawnX, LEVEL3_CONFIG.spawnY);
    ...
    // ---- BGM ----
    AudioManager.get(this).playBgm(AUDIO.BGM_LEVEL3);
  ...
  update(_t: number, delta: number) {
    ...
    // Boss trigger
    if (!this.bossTriggered && this.player.x >= LEVEL3_CONFIG.bossTriggerX) {
      this.spawnBoss();
    }
    ...
    // Death plane
    if (!this.player.isDead && this.player.y > LEVEL3_CONFIG.deathY) {
      this.respawnPlayer();
    }
  }
  ...
  private buildCollectibles(): void {
    ...
    for (const s of LEVEL3_STONES) {
      const stone = new Collectible({
        scene: this,
        x: s.x,
        y: s.y,
        type: 'stone',
        textureKey: TEX.ITEM_BATUKRISTAL,
        value: 1,
      });
      stone.setDepth(4);
  ...
  private buildCheckpoints(): void {
    for (const cp of LEVEL3_CHECKPOINTS) {
  ...
  private buildWaterZones(): void {
    for (const wz of LEVEL3_WATERZONES) {
      const zone = this.add.rectangle(wz.x, wz.y, wz.w, wz.h, 0x4dc6ff, 0.15);
      zone.setDepth(1);
  ...
  private buildLavaZones(): void {
    for (const lz of LEVEL3_LAVAZONES) {
  ...
  private collectStone(stone: Collectible): void {
    ...
    this.cameras.main.flash(150, 77, 198, 255, false);
  }
  ...
  private respawnPlayer(): void {
    this.cameras.main.flash(200, 20, 60, 80);
  ...
  private spawnDeathBurst(x: number, y: number): void {
    for (let i = 0; i < 10; i++) {
      const p = this.add.circle(x, y - 20, Phaser.Math.Between(2, 4), 0x4dc6ff, 1).setDepth(9);
  ...
  private launchHud(): void {
    const hudData = {
      ...
      stoneTarget: LEVEL_TARGETS[3].stones,
      stoneAccent: LEVEL_ACCENT,
      mode: this.player.mode,
      unlocked: { fire: true, water: true },
    };
  ...
  private spawnBoss(): void {
    ...
    const bx = LEVEL3_CONFIG.bossTriggerX + 350;
    this.boss = new Boss({
      scene: this,
      x: bx,
      y: LEVEL3_CONFIG.groundY - 200,
      type: 'es',
      hpMax: 30,
      chaseRange: 800,
      stopRange: 100,
    ...
    this.events.emit('hud:bossShow', { name: 'Penjaga Es' });
    AudioManager.get(this).playSfx(this, AUDIO.SFX_VICTORY, 0.4);

    // Arena wall
    this.addCollider(LEVEL3_CONFIG.bossTriggerX - 40, LEVEL3_CONFIG.groundY - 200, 40, 500);
  }

  private onBossDefeated(): void {
    this.events.emit('hud:bossHide');
    AudioManager.get(this).playSfx(this, AUDIO.SFX_VICTORY, 0.9);
    this.cameras.main.flash(300, 77, 198, 255);
    if (this.eggSpawned) return;
    this.eggSpawned = true;
    const ex = this.boss ? this.boss.x : LEVEL3_CONFIG.bossTriggerX + 350;
    this.egg = new Egg({
      scene: this,
      x: ex,
      y: LEVEL3_CONFIG.groundY,
      type: 'es',
      hpMax: 5,
      renderHeight: 90,
      accentColor: LEVEL_ACCENT,
  ...
  private spawnFinishPortal(): void {
    const px = (this.egg ? this.egg.x : LEVEL3_CONFIG.exitX) + 200;
    const py = LEVEL3_CONFIG.groundY - 60;
    const portal = this.add.container(px, py).setDepth(6);
    const g = this.add.graphics();
    g.fillStyle(0x4dc6ff, 0.25); g.fillCircle(0, 0, 70);
    g.fillStyle(0x80d8ff, 0.35); g.fillCircle(0, 0, 50);
  ...
  private completeLevel(): void {
    ...
    this.cameras.main.once(Phaser.Cameras.Scene2D.Events.FADE_OUT_COMPLETE, () => {
      ...
      this.scene.start(SCENE.HASIL, {
        level: 3,
        coin: this.skor_koin,
        coinTarget: LEVEL_TARGETS[3].coin,
        stone: this.batu_terkumpul,
        stoneTarget: LEVEL_TARGETS[3].stones,
      });
    });
  ...
  private updateCameraVerticalLock(): void {
    ...
    cam.setZoom(zoom);
    cam.setBounds(0, -1000, LEVEL3_CONFIG.width, DESIGN_VIEW_HEIGHT + 2000);
    this.lockCameraY();
  }

  private lockCameraY(): void {
    const cam = this.cameras.main;
    const groundedScrollY = LEVEL3_CONFIG.groundY - DESIGN_VIEW_HEIGHT * GROUND_SCREEN_FRAC;
  ...
  private lockCameraX(): void {
    ...
    targetX = Phaser.Math.Clamp(targetX, 0, Math.max(0, LEVEL3_CONFIG.width - visibleW));
